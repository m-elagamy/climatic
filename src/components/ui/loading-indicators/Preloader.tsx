    ...
    <section className="preloader fixed inset-0 -z-[1] flex items-center justify-center bg-gradient-to-tl from-[#1c2670] to-[#3a9ecc]">
      ...
      <article className="relative">
        ...
        <svg
          ...
          height="10px"
          viewBox="0 0 10 10"
          className="z-1 relative mx-auto size-[110px] animate-float"
        >
          <path
            fill="none"
            className="fill-[#F0F0F0] stroke-[#E0E0E0]"
        ...
        <div className="absolute bottom-5 left-1/2 -translate-x-1/2 *:float-left *:ml-[4px] *:h-[10px] *:w-[3px] *:animate-drop *:rounded-b-[6px] *:bg-[#36bfff] md:bottom-6">
          {DROP_DELAYS.map((delay, index) => (
            <span key={index} style={{ animationDelay: `${delay}ms` }}></span>
          ))}
        </div>
        <p className="pt-4 text-center text-xs font-semibold uppercase tracking-wider text-primary md:text-sm">
          Just a moment, scanning the clouds
